                ...
                <div class="row">
                    <div class="col-md-3">
                        <a href="/event/{{$event->url}}"><img class="img-responsive"
                                                              src="/images/events/{{$event->id}}.png"></a>
                    </div>
                    <div class="col-md-6">
                        <a href="/event/{{$event->url}}"><h3>{{$event->name_e}}</h3><br>{{$start_date}} to {{$end_date}}
                        </a>
                    </div>
                    <div class="col-md-3">
                        <?php
                        ...
                        @if (isset($user))
                            @if(isset($event->user_event))
                                Rank: <input type="number" id="rank-event-{{$event->id}}"
                                             value="{{$event->user_event->rank}}"><br>
                                Points: <input type="number" id="points-event-{{$event->id}}"
                                               value="{{$event->user_event->points}}">
                                <button class="btn btn-primary update-event" data-id="{{$event->id}}">Update</button>
                            @else
                                <div class="user-event">
                                    <button class="btn btn-success add-event" data-id="{{$event->id}}">Add Event
                                    </button>
                                </div>
                            @endif
                        @endif
