...
@section('content')
    <div class="container">
        ...
        ?>
        <h1>
            <div class="col-xs-6"> {!! Form::text('name_e',$chapter->name_e,['class'=>'form-control name_e','id'=>$chapter->id]) !!} </div>
            - {{$story->name_e}} <small>({{$count}} slides)</small>

            @if ($chapter->complete == 1)
                <button id="display-chapter" class="btn btn-danger chapter-display" data-id="{{$chapter->id}}"
                        data-display="hide">Hide
                </button></h1>
        @else
            <button id="display-chapter" class="btn btn-success chapter-display" data-id="{{$chapter->id}}"
            ...
            <div class="row">
                <div class="col-md-6">
                    <img class="img-responsive"
                         src="/images/translate/{{$event_type}}/{{$story->id}}/{{$chapter->chapter}}_{{$slide_number}}.{{$chapter->file_type}}">
                </div>
                <div class="col-md-6">
                ...
                <!-- italics button -->
                    ...
                    <br>
                    {!! Form::textarea('notes', $slide->notes,['class'=>'form-control ajaxTest', 'id'=>'n-'.$slide->id, 'rows' => 3, 'cols' => 100, 'placeholder'=>'Notes']) !!}
                    <br>
                    {!! Form::hidden('slide_id', $slide->id) !!}
                    {!! Form::hidden('chapter_id', $chapter->id) !!}
                    {!! Form::hidden('story_id', $story->id) !!}
                    {!! Form::select('boy_id', $boys,$slide->boy_id, ['class'=>'dropdownAjax','id'=>'s-'.$slide->id, 'placeholder' => 'Speaker']) !!}   {{$slide->slide}}
